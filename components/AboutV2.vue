<template>
  <div
    class="container mx-auto mt-[120px] flex items-center justify-center gap-8"
  >
    <img src="~/assets/section-peoples (1).png" alt="a woman" />

    <div class="flex flex-col gap-4">
      <h3 class="font-bold text-[56px]">Sekolah Alam Insan Mulia Berkarya</h3>

      <p class="font-normal text-[20px]">
        Sekolah Kami merupakan sekolah informal yang dikelola untuk membantu
        anak mengembangkan bakat dan kemampuannya. Dengan memberikan kurikulum
        terbaik agar anak bisa memilih minatnya dan fokus mengembangkan minat
        tersebut. Sekolah ini didirikan sejak tahun 1989 dan terus berkembang
      </p>

      <a href="" class="btn-style-9 w-[186px]">Mulai Belajar</a>
    </div>
  </div>
</template>
